class HoboMigration9 < ActiveRecord::Migration
  def self.up
    add_column :exams, :finished, :boolean
    remove_column :exams, :current_response_index
  ...
  def self.down
    remove_column :exams, :finished
    add_column :exams, :current_response_index, :integer, :default => 0
